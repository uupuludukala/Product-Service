...
@Component
public class ProductCategoryGetResourceAssembler extends AbstractProductCategoryResourceAssembler {
	@Override
	protected String getSelfLink(String id) {
		if (!StringUtils.isEmpty(id)) {
			...
			if (!builder.build().toString().endsWith("/productCategories")) {
				String branchCurrentURI = builder.build().toString();
				String branchURI = branchCurrentURI.substring(0, branchCurrentURI.indexOf("/productCategories") + 9);
