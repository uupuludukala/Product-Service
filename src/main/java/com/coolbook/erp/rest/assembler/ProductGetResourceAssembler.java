...
@Component
public class ProductGetResourceAssembler extends AbstractProductResourceAssembler {
	@Override
	protected String getSelfLink(String id) {
		if (!StringUtils.isEmpty(id)) {
			...
			if (!builder.build().toString().endsWith("/products")) {
				String branchCurrentURI = builder.build().toString();
				String branchURI = branchCurrentURI.substring(0, branchCurrentURI.indexOf("/products") + 9);
