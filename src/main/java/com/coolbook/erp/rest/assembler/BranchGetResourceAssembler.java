...
@Component
public class BranchGetResourceAssembler extends AbstractBranchResourceAssembler{

	@Override
	protected String getBranchSelfLink(String id) {
		if (!StringUtils.isEmpty(id)) {
			UriComponentsBuilder builder = ServletUriComponentsBuilder.fromHttpUrl(requestURI).query(null);
			if (!builder.build().toString().endsWith("/branchess")) {
				String branchCurrentURI = builder.build().toString();
				String branchURI = branchCurrentURI.substring(0, branchCurrentURI.indexOf("/branchess") + 9);
