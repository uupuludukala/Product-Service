...
@Component
public class EmployeeGetResourceAssembler  extends AbstractEmployeeResourceAssembler{
    @Override
    protected String getSelfLink(String id) {
        if (!StringUtils.isEmpty(id)) {
            ...
            if (!builder.build().toString().endsWith("/employeess")) {
                String branchCurrentURI = builder.build().toString();
                String branchURI = branchCurrentURI.substring(0, branchCurrentURI.indexOf("/employees") + 9);
